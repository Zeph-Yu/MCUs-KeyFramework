...
{
    GPIO_PinState state = HAL_GPIO_ReadPin(KeyTable[KeyID].Port, KeyTable[KeyID].Pin);
    return (state == KeyTable[KeyID].TriggerLevel) ? KEY_PRESSED : KEY_UNPRESSED;
}
...
/*===========================================================================
 * Key_Tick — 按键扫描状态机 (由 SysTick ISR 每 1ms 调用一次)
 *
